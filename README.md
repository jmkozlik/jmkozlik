...
**I build governance and oversight infrastructure for agent systems** — because deployment-grade oversight is a systems-engineering problem.

CS & Applied Math @ NJIT '27 · NSF S-STEM Scholar · Two-time founder

</div>

## Now

- **AI Application Engineer Intern** @ Southern Company's AI Center of Excellence — enterprise LLM RAG over 100K+ sensitive documents (GCP, Google ADK)
- **Co-Founder** @ [Trainsafe](https://www.trainsafe.ai) — multi-tenant agent-workflow platform: a 19-node async LLM engine with human-in-the-loop pauses, dual-store agent memory, and an MCP endpoint
- Grinding [Neural Networks: Zero to Hero](https://karpathy.ai/zero-to-hero.html) — every chapter re-implemented on my own from-scratch engine
...
`Python` `C++` `C` `Java` `FastAPI` `asyncio` `MCP` `Docker` `PostgreSQL` `GCP`
